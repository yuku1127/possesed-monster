...
class Player
{
    constructor(x,y)
    {
        ...
        this.vx=0;//x速度
        this.vy=0;//y速度
        this.anim=0;//プレイヤーのアニメーション番号
        this.snum=0;//data.jsのsprite配列に対応した画像番号
        this.acou=0;//点滅時間を連続する画像を管理するカウント(行番号545)
        this.dirc=0;//方向
        this.jump=0;//ジャンプ時間を管理する変数(行番号283参照)
        this.reload=0;//リロード時間
        this.type=0;//プレイヤーのタイプ（どのモンスターに憑依しているか）
        this.newtype=0;//次に憑依するモンスターのタイプ
        this.newhp=0;//次のモンスターのhp
        this.muteki=0;//無敵時間
        this.guard=false;//バリア状態
        this.trans=0;//次のモンスターに憑依する移行時間
        this.r=0;//プレイヤー半径
        this.w=0;//プレイヤーの幅
        this.h=0;//プレイヤーの高さ
        this.mhp=100;//プレイヤー最大hp(初期値１００)
        this.hp=this.mhp;//現在のhp
        this.prehp=0;//モンスターに憑依する前のプレイヤーのhpを保存
        this.swimflag=false;//泳ぎ中のフラグ
        this.tamaflag=0;//射撃モーションのフラグ(行番号377)
        this.flag1=0;
        this.flag2=0;
        this.angle=0;
        this.chf=false;
